...
PLOT_CONFIG = {
    ...
    'colors': {
        'astar': '#3498db',
        'ml': '#2ecc71',
        'astar_dynamic': '#2980b9',
        'ml_dynamic': '#27ae60'
    },
    'display': {
        'astar': 'A*',
        'ml': 'ML',
        'astar_dynamic': 'A* (Dynamic)',
        'ml_dynamic': 'ML (Dynamic)'
    }
}

ALGO_KEYMAP = {
    'astar': ['a*', 'astar', 'a_star', 'a star', 'a-star'],
    'astar_dynamic': ['a* dynamic', 'astar dynamic', 'a*dynamic', 'a_star_dynamic', 'a star dynamic'],
    'ml': ['ml', 'ml heuristic', 'ml_static', 'ml-static', 'ml_static_heuristic'],
    'ml_dynamic': ['ml dynamic', 'ml-dynamic', 'ml_dynamic', 'ml_dynamic_heuristic']
}

PAIRWISE_PAIRS = [('astar', 'ml'), ('astar_dynamic', 'ml_dynamic')]
    ...
    for key, variants in ALGO_KEYMAP.items():
        for v in variants:
            if v == s0:
                return key
    if 'ml' in s0:
        if 'dyn' in s0 or 'dynamic' in s0:
            return 'ml_dynamic'
        return 'ml'
    if 'a*' in s0 or 'astar' in s0 or 'a star' in s0:
        if 'dyn' in s0 or 'dynamic' in s0:
            return 'astar_dynamic'
        return 'astar'
    return s0
...
def _produce_pairwise_plots_for_subset(df_pair, a_key, b_key, metric_nodes, metric_time, metric_cost, pair_dir):
    ...
    # nodes vs path cost scatter with colored points + regression per algo
    if metric_nodes is not None and metric_cost is not None:
        ...
        for algo in [a_key, b_key]:
            ...
            if sub.shape[0] >= 2:
                ...
                plt.plot(xs, ys, linestyle='--', color=PLOT_CONFIG['colors'][algo], label=f"{PLOT_CONFIG['display'][algo]} fit")
                if SCIPY:
                    try:
                    ...
                    try:
                        ...
                        r, p = float('nan'), float('nan')
                metrics_summary.append({'algorithm': PLOT_CONFIG['display'][algo], 'slope': slope, 'r': r, 'p': p, 'n': int(mask.sum())})
        ...
        txt_lines = [f"{m['algorithm']}: slope={m['slope']:.3f}, r={m['r']:.3f}, p={'{:.3g}'.format(m['p']) if m['p'] is not None else 'N/A'}" for m in metrics_summary]
        if txt_lines:
            plt.gcf().text(0.02, 0.02, "\n".join(txt_lines), fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
...
def _produce_per_algo_plots_for_subset(sub, algo, metric_nodes, metric_cost, metric_time, algo_dir):
    # Nodes vs Path Cost scatter + regression
    if metric_cost is not None:
        ...
        if mask.sum() >= 2:
            ...
            slope, intercept = float(coef[0]), float(coef[1])
            xs = np.linspace(np.nanmin(x[mask]), np.nanmax(x[mask]), 100)
            plt.plot(xs, slope * xs + intercept, color='red', linestyle='--', label=f'fit slope={slope:.3f}')
            r, p = safe_pearson(x, y)
            plt.gcf().text(0.02, 0.02, f"slope={slope:.3f}, r={r:.3f}, p={'{:.3g}'.format(p) if not math.isnan(p) else 'N/A'}", fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
        plt.xlabel(metric_cost); plt.ylabel(metric_nodes)
...
def plot_paired_scatter_and_stats(df: pd.DataFrame, baseline_key: str, test_key: str, metric_col: str, output_dir: str):
    ...
    try:
        ...
        plt.plot(xs, slope * xs + intercept, color='red', linestyle='-', linewidth=1.6, alpha=0.8, label=f'fit slope={slope:.3f}')
        r, p = safe_pearson(x, y)
    except Exception:
...
def plot_tradeoff_time_vs_path(df: pd.DataFrame, output_dir: str):
    ...
    df2['env_mode'] = df2['__algo_norm'].apply(lambda x: 'dynamic' if 'dynamic' in x else 'static')
    ...
    if mask.sum() >= 2:
        ...
        slope = float(coef[0])
        xs = np.linspace(np.nanmin(x[mask]), np.nanmax(x[mask]), 100)
        plt.plot(xs, slope*xs + coef[1], color='black', linestyle='--', label=f'fit slope={slope:.3f}')
        r, p = safe_pearson(x, y)
        plt.gcf().text(0.02, 0.02, f"slope={slope:.3f}, r={r:.3f}", fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
